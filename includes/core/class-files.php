...
if ( ! class_exists( 'um\core\Files' ) ) {
	...
	class Files {
		...
		function ajax_resize_image() {
			...
			$output = UM()->uploader()->resize_image( $image_path, $src, $key, $user_id, $coord, $index, $text,$text1,$text2,$text3,$over_url1,$over_url2,$over_url3 );
		 ...
		 * @param $type
		 */
		function delete_core_user_photo( $user_id, $type, $index = null ) {
			 ...
			 */
			if ($type === 'cover_photo')
                do_action( "um_after_remove_{$type}", $user_id, $index );
            else

...
function um_profile_header_cover_area( $args ) {
	...
	global $current_user;
	global $over_index;
    $cover_array = get_user_meta( esc_attr(um_profile_id()), 'cover_photo', true );
    $cover_count = 0;
    if ( is_array( $cover_array ) )
        $cover_count = count( get_user_meta( esc_attr(um_profile_id()), 'cover_photo', true ) );
    $limit = 10;
    $has_cover = false;
    $cover_plus = false;
    ...
    if ( $args['cover_enabled'] == 1 ) {
        ...
        for ($index = 0; $index < $limit; $index++) {
            ...
            $overlay = '<span class="um-cover-overlay">
                    <span class="um-cover-overlay-s">
                        <ins>
						    ...
						    <span class="um-cover-overlay-t">' . __( 'Change your cover photo and over_text', 'ultimate-member' ) . '</span>
                        </ins>
                    </span>
                </span>';

            $has_cover = um_user('cover_photo', $index);
            if ( !UM()->fields()->editing ) {
                if ( !$has_cover ) continue;
                if ( !$has_cover && $index < $limit -1) {
                    $limit = $index + 1;
                }
            }
            $over_index =$index;
            ...
            ?>
            <div class="um-cover <?php if ($has_cover || ($default_cover && $default_cover['url'])) echo 'has-cover'; ?>"
                 data-user_id="<?php echo esc_attr(um_profile_id()); ?>"
                 data-ratio="<?php echo esc_attr($args['cover_ratio']); ?>" data-index="<?php echo $index; ?>" id="um-cover<?php echo $index; ?>" style="height:500px">
                ...
                if ( UM()->fields()->editing ) {
                    ...
                    $hide_remove = um_user( 'cover_photo', $index ) ? false : ' style="display:none;"';

                    $text = ! um_user( 'cover_photo', $index ) ? __( 'Upload a cover photo', 'ultimate-member' ) : __( 'Change cover photo or text', 'ultimate-member' ) ;
                ...
                <div class="um-cover-e" data-ratio="<?php echo esc_attr( $args['cover_ratio'] ); ?>" onclick=show(<?php echo $index; ?>); style="height:500px;">

                    <?php if ( um_user( 'cover_photo', $index ) ) {
                        ...
                        echo um_user( 'cover_photo', $index, $size );
						?>
						<script>
						document.getElementsByClassName('um-cover-e').style.height = "1000px";
						</script>
						<?php
                        $over_padding=-330;
                        for($over_num = 0;$over_num <4;$over_num ++)
                        {
                            if($over_num == 0) {
                                ?>
                                <a  class="over_display">
                                <?php
                                echo "<h3  index='$index' id='overheader$index'  >";
                                echo um_user('over_text', $index, $over_num);
                                echo "</h3></a><div class='um-over-text-output'>";
                                $over_padding = $over_padding + 100;
                            }
                            else{
								// if(um_user('over_url',$index,$over_num))
								// {
									echo "<a id='overaddress$index$over_num' href=";
									echo um_user('over_url',$index,$over_num);
									echo "> <h5 class='over_ma' index='$index' id='overtext$index$over_num' style='visibility: hidden; display:none;' >";
									echo um_user('over_text', $index, $over_num);
									echo "</h5></a>";
									$over_padding = $over_padding + 100;
								// }
            ...
            <?php
            if ( $has_cover ) $cover_count--;
            if ( UM()->fields()->editing ) {
                if ( $cover_plus ) break;
                if ( $cover_count == 0 && !$has_cover ) break;
                if ( $cover_count == 0 ) $cover_plus = true;
			}
        }
        ?>
        ...
        <script>
            ...
            coun_limit = <?php echo count( get_user_meta( esc_attr(um_profile_id()), 'cover_photo', true ) ); ?>;
            limit = <?php echo $limit; ?>;
			// for(var i = 0; i < limit; i++)
			// {
				// document.getElementsByClassName('um-cover-e')[i].style.height = "500px";
			//  document.getElementsByClassName('um-cover-e')[i].style.display = "none";
			// }
			in_num = 2 ;
            function show(a){
                ...
                switch (a){
                    case 0:
                        if(in_num == '2' && document.getElementById('overtext01').style.visibility == "hidden") {

                            for(var k = 1;k <4; k++)
                            {
								document.getElementById('overtext'+a+k).style.visibility = "visible";
								document.getElementById('overtext'+a+k).style.display = "block";
								
							}
							for(var k = 1;k <4; k++)
                            {
								if(document.getElementById('overtext'+a+k).innerHTML =="")
								{
									document.getElementById('overaddress'+a+k).style.pointerEvents = "none";
										
								}
								
							}
                            document.getElementById('overheader'+a).style.visibility = "hidden";
							document.getElementById('um-cover'+a).style.marginBottom = "150px";
							//document.getElementById('um-cover'+a).style.clipPath = "polygon(0 0, 100% 0, 100% 85%, 0 85%)";
                            in_num =1;
                            break;
                        }
                        if(in_num == '1' && document.getElementById('overtext01').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                                    document.getElementById('overtext'+i+j).style.visibility = "hidden";
                                }
                                document.getElementById('overheader'+i).style.visibility = "visible";
                                document.getElementById('um-cover'+i).style.zIndex = "0";
								document.getElementById('um-cover'+i).style.marginBottom = "0px";
                            ...
                            for(var k = 1; k<4;k++){
								document.getElementById('overtext'+a+k).style.visibility = "hidden";
								document.getElementById('overtext'+a+k).style.display = "none";
                            }
                            document.getElementById('overheader'+a).style.visibility = "visible";
							document.getElementById('um-cover'+a).style.zIndex = "0";
							document.getElementById('um-cover'+a).style.marginBottom = "0px";
							//document.getElementById('um-cover'+a).style.clipPath = "polygon(0 0, 100% 0, 100% 50%, 0 50%)";
							in_num  = 2;
						}
						
                        break;
                    case 1:
                        if(in_num == '2' && document.getElementById('overtext11').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext11').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 2:
                        if(in_num == '2' && document.getElementById('overtext21').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext21').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 3:
                        if(in_num == '2' && document.getElementById('overtext31').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext31').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 4:
                        if(in_num == '2' && document.getElementById('overtext41').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext41').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 5:
                        if(in_num == '2' && document.getElementById('overtext51').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext51').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 6:
                        if(in_num == '2' && document.getElementById('overtext61').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext61').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 7:
                        if(in_num == '2' && document.getElementById('overtext71').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext71').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 8:
                        if(in_num == '2' && document.getElementById('overtext81').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext81').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {
                        ...
                        break;
                    case 9:
                        if(in_num == '2' && document.getElementById('overtext91').style.visibility == "hidden") {
                        ...
                        if(in_num == '1' && document.getElementById('overtext91').style.visibility == "hidden"){
                            for (var i =0; i<coun_limit; i++)
                            {
                                for(var j = 1; j<4; j++)
                                {

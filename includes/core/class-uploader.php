...
if ( ! class_exists( 'um\core\Uploader' ) ) {
	...
	class Uploader {
		...
		function __construct() {
			...
			add_filter( 'um_upload_image_process__profile_photo', array( $this, 'profile_photo' ), 10, 15 );
			add_filter( 'um_upload_image_process__cover_photo', array( $this, 'cover_photo' ), 10, 15 );
			add_action( 'um_upload_stream_image_process', array( $this, 'stream_photo' ), 10, 15 );
		 ...
		 * @return array
		 */
		public function profile_photo( $response, $image_path, $src, $key, $user_id, $coord, $crop, $index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 ) {
		 ...
		 * @return array
		 */
		public function cover_photo( $response, $image_path, $src, $key, $user_id, $coord, $crop, $index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 ) {
		    ...
		    if($coord) {
                ...
                //refresh image_path to make temporary image permanently after upload
                $image_path = pathinfo($image_path, PATHINFO_DIRNAME) . DIRECTORY_SEPARATOR . $key . $index . '.' . pathinfo($image_path, PATHINFO_EXTENSION);
                ...
                if (!is_wp_error($image)) {
                    ...
                    $src = str_replace('/' . $key . '_temp.', '/' . $key . $index . '.', $src);
                    ...
                    $cover_array = get_user_meta($this->user_id, $key, true);
                    $cover_array = !empty($cover_array) ? $cover_array : array();
                    $cover_array[$index] = array("name" => wp_basename(wp_basename($image_path)), "comment0" => $text, "comment1" => $text1, "comment2" => $text2, "comment3" => $text3, "over_url1" => $over_url1, "over_url2" => $over_url2, "over_url3" => $over_url3);

                    update_user_meta($this->user_id, $key, $cover_array);
                ...
                $cover_array = get_user_meta($this->user_id, $key, true);
                $cover_array = !empty($cover_array) ? $cover_array : array();
                $cover_array[$index] = array("name" => '', "comment0" => $text, "comment1" => $text1, "comment2" => $text2, "comment3" => $text3, "over_url1" => $over_url1, "over_url2" => $over_url2, "over_url3" => $over_url3);
                update_user_meta($this->user_id, $key, $cover_array);
                $cover_array[$index] = array("name" => wp_basename( wp_basename( $image_path ) ),"comment0" => $text, "comment1" => $text1, "comment2" => $text2, "comment3" => $text3, "over_url1" => $over_url1, "over_url2" => $over_url2, "over_url3" => $over_url3);
                update_user_meta( $this->user_id, $key, $cover_array );
                delete_user_meta($this->user_id, "{$key}_metadata_temp");
		 ...
		 * @return array
		 */
		public function stream_photo( $response, $image_path, $src, $key, $user_id, $coord, $crop, $index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 ) {
            $image = wp_get_image_editor( $image_path ); // Return an implementation that extends WP_Image_Editor
		 ...
		 * @return array
		 */
		public function resize_image( $image_path, $src, $key, $user_id, $coord, $index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 ) {
			$crop = explode( ',', $coord );
			...
			$response = apply_filters( "um_upload_image_process__{$key}", $response, $image_path, $src, $key, $user_id, $coord, $crop, $index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 );

			if ( ! in_array( $key, array( 'profile_photo', 'cover_photo' ) ) ) {
				$response = apply_filters( 'um_upload_stream_image_process', $response, $image_path, $src, $key, $user_id, $coord, $crop,$index, $text, $text1,$text2,$text3, $over_url1,$over_url2,$over_url3 );
			}

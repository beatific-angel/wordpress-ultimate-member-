 ...
 * @return bool|string
 */
function um_get_cover_uri( $image, $index, $attrs ) {
    $cover_array = get_user_meta( um_user( 'ID' ), 'cover_photo', true );
    if ( !is_array( $cover_array ) ) return false;

    $image = $cover_array[$index]['name'];
    ...
    if ( is_multisite() ) {
        ...
        if ( file_exists( $multisite_fix_dir . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}{$ext}" ) ) {
            $uri_common = $multisite_fix_url . um_user( 'ID' ) . "/cover_photo{$index}{$ext}?" . current_time( 'timestamp' );
        }

        if ( file_exists( $multisite_fix_dir . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}-{$attrs}{$ext}" ) ) {
            $uri_common = $multisite_fix_url . um_user( 'ID' ) . "/cover_photo{$index}-{$attrs}{$ext}?" . current_time( 'timestamp' );
        }elseif ( file_exists( $multisite_fix_dir . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}-{$attrs}x{$height}{$ext}" ) ) {
            $uri_common = $multisite_fix_url . um_user( 'ID' ) . "/cover_photo{$index}-{$attrs}x{$height}{$ext}?". current_time( 'timestamp' );
        }
    }

    if ( file_exists( UM()->uploader()->get_upload_base_dir() . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}{$ext}" ) ) {
        $uri = UM()->uploader()->get_upload_base_url() . um_user( 'ID' ) . "/cover_photo{$index}{$ext}?" . current_time( 'timestamp' );
    }
    if ( file_exists( UM()->uploader()->get_upload_base_dir() . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}-{$attrs}{$ext}" ) ) {
        $uri = UM()->uploader()->get_upload_base_url() . um_user( 'ID' ) . "/cover_photo{$index}-{$attrs}{$ext}?" . current_time( 'timestamp' );
    }elseif ( file_exists( UM()->uploader()->get_upload_base_dir() . um_user( 'ID' ) . DIRECTORY_SEPARATOR . "cover_photo{$index}-{$attrs}x{$height}{$ext}" ) ) {
        $uri = UM()->uploader()->get_upload_base_url() . um_user( 'ID' ) . "/cover_photo{$index}-{$attrs}x{$height}{$ext}?". current_time( 'timestamp' );
    }
 ...
 * @return bool|string
 */
function um_get_over_uri( $over_text, $index, $attrs ) {
    $over_array = get_user_meta(um_user( 'ID' ), 'cover_photo', true );
    if ( !is_array( $over_array ) ) return false;
    $over_text = array();
    for ($i = 0;$i < 3; $i++){
            $over_text = $over_array[$index]["comment{$attrs}"];
    }
 ...
 * @return bool|string
 */
function um_get_over_text_uri( $over_url, $index, $attrs ) {
    $over_array = get_user_meta(um_user( 'ID' ), 'cover_photo', true );
    if ( !is_array( $over_array ) ) return false;
    $over_url = array();
    for ($i = 0;$i < 3; $i++){
        $over_url = $over_array[$index]["over_url{$attrs}"];
    }
 ...
 * @param $data
 * @param null $attrs
 * @param null $size( for 'cover_photo' )
 *
 * @return string|array
 */
function um_user( $data, $attrs = null, $size = null ) {
    ...
    switch ($data) {
        ...
        case 'cover_photo':
            ...
            if ( um_profile( 'cover_photo' ) ) {
                $cover_uri = um_get_cover_uri( um_profile( 'cover_photo' ), $attrs, $size );
             ...
             * ?>
             */
            $cover_uri = apply_filters( 'um_user_cover_photo_uri__filter', $cover_uri, $is_default, $size );

            $alt = um_profile( 'nickname' );

            $cover_html = $cover_uri ? '<img src="' . esc_attr( $cover_uri ) . '" alt="' . esc_attr( $alt ) . '" />' : '';

            $cover_html = apply_filters( 'um_user_cover_photo_html__filter', $cover_html, $cover_uri, $alt, $is_default, $size );
            return $cover_html;
        ...
        case 'over_text':
            ...
            if ( um_profile( 'cover_photo' ) )
            {
                ...
                $over_uri = um_get_over_uri( um_profile( 'cover_photo' ), $attrs, $size );
            }

            $over_html = $over_uri ? esc_attr( $over_uri ) : '';
            return $over_html;
        case 'over_url':
            $is_default = false;
            ...
            if ( um_profile( 'cover_photo' ) )
            {
                ...
                $over_uri = um_get_over_text_uri( um_profile( 'cover_photo' ), $attrs, $size );
            }

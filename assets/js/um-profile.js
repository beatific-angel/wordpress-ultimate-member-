jQuery(document).ready(function() {
	...
	jQuery(document.body).on('click', '.um-reset-cover-photo', function(e){
        ...
        var index = 0;
        if ( jQuery(this).parents('.um-trigger-menu-on-click').attr('data-index')  ) {
            index = jQuery(this).parents('.um-trigger-menu-on-click').attr('data-index');
        }
		jQuery('.um-cover-overlay:eq(' + index + ')').hide();

        jQuery('.um-cover-e:eq(' + index + ')').html('<a href="javascript:void(0);" class="um-cover-add" style="height: 370px;"><span class="um-cover-add-i"><i class="um-icon-plus um-tip-n" original-title="Upload a cover photo"></i></span></a>');
    ...
    // jQuery('.over_display').click( function(e) {
    //     e.preventDefault();
		// alert("dfsdf");
    //
    // });
    // jQuery( document.body ).on( 'click', '.over_display', function(e){
    //     alert("dfsdf");
    //     e.preventDefault();
    // });
